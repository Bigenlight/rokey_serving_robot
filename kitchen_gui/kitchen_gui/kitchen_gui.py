...
=======
>>>>>>> e9d19254b784a420d7e87651e28bf857e21e49aa
import sys
import threading
import queue
...
class KitchenGUINode(Node):
    ...
    def send_emergency_stop(self):
        ...
        # amr_status에 "emergency_stop" 퍼블리시
        status_msg = String()
        status_msg.data = "emergency_stop"
        self.amr_status_publisher.publish(status_msg)
        self.get_logger().info("amr_status에 'emergency_stop' 상태를 퍼블리시했습니다.")
    ...
    def send_navigate_goal_to_position(self, position):
        ...
        send_goal_future.add_done_callback(lambda future: self.navigate_to_pose_action_goal(future, "주방 복귀"))

        # amr_status에 "return_to_kitchen" 퍼블리시
        status_msg = String()
        status_msg.data = "return_to_kitchen"
        self.amr_status_publisher.publish(status_msg)
    ...
    def send_navigate_goal(self, table_name):
        ...
        send_goal_future = self.navigate_to_pose_action_client.send_goal_async(
            goal_msg,
            feedback_callback=self.navigate_feedback_callback)
        send_goal_future.add_done_callback(lambda future: self.goal_response_callback(future, table_name))

        # amr_status에 "delivering" 퍼블리시
        status_msg = String()
        status_msg.data = "delivering"
        self.amr_status_publisher.publish(status_msg)
        self.get_logger().info(f"amr_status에 'delivering' 상태를 퍼블리시했습니다. (목표: {table_name})")
    ...
    def goal_response_callback(self, future, action_name):
        ...
        if not goal_handle.accepted:
            ...
            self.gui.show_standby()  # 목표 거부 시 이미지로 복귀
            return
